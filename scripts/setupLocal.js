...
async function main() {
  ...
  console.log(`Approving ${ethers.formatEther(rewardAmount)} XAUT for reward deposit...`);
  await token.approve(stakingAddress, rewardAmount);
  console.log("Depositing rewards into contract...");
  await staking.depositRewards(rewardAmount);
  console.log("Rewards deposited:", ethers.formatEther(rewardAmount), "XAUT");
  ...
  console.log("\n=== Setup Complete ===");
  console.log("\nAccount Balances:");
  console.log("Owner:", ethers.formatEther(await token.balanceOf(deployer.address)), "XAUT");
  ...
  console.log("\nReward Pool:", ethers.formatEther(await staking.rewardPool()), "XAUT");

  console.log("\n=== Private Keys (for MetaMask import) ===");
  console.log("WARNING: These are test keys. Never use on mainnet!");
  console.log("Check Hardhat node output for private keys");
